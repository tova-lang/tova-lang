# Plan: Comprehensive Lux Language Documentation
...
- What Lux is and its design philosophy
...
- Installing the CLI (`npm install -g lux-lang` or equivalent)
- `lux new my-app` walkthrough
- Project structure explanation
- `lux dev`, `lux build`, `lux run` quick usage
...
- **Concept**: `shared {}`, `server {}`, `client {}` in one `.lux` file
...
- **Migrations**: `lux migrate:create`, `lux migrate:up`, `lux migrate:status`
...
- `lux new <name>` — scaffold project
- `lux dev [dir]` — development server with hot reload
- `lux build [dir]` — compile to JavaScript
- `lux run <file>` — compile and execute
- `lux migrate:create <name>` — create migration
- `lux migrate:up [file]` — run migrations
...
- `throw` is NOT a keyword — Lux has no throw statement
...
- CLI: `bin/lux.js`
- Examples: `examples/*.lux`

## Verification

1. Review each doc file for accuracy against source code
2. Verify all code examples compile with `lux run` or `lux build`
