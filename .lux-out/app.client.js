...
const high_count = createComputed(() => len(notes().filter((n) => (n.priority == "High")).map((n) => n)));
...
function remove_note(id) {
  setNotes(notes().filter((n) => (n.id != id)).map((n) => n));
}
